...
const CardBar = () => {
  ...
  <div className="flex items-center justify-between px-5 py-1 transition duration-200 ease-in">
      ...
      {filterOpen && (   <div className={`flex-col gap-10 ${filterOpen ? "flex" : "hidden"} fixed bottom-0 border-t-4 border-[#6D4AFF] left-0 right-0 z-20 bg-white px-8 py-6 rounded-t-4xl  shadow-md h-[78vh] `}>
     <div className="grid grid-cols-1 gap-7 ">
      <div className="flex justify-between items-center ">
        <h1 className="text-lg text-[#6D4AFF] font-medium">Filters </h1>
      <X onClick={()=>setFilterOpen(false)} className=" right-3 -top-2 h-8 text-[#6D4AFF] font-bold" />
       ...
       <div className="flex flex-col min-w-0 gap-2 transition duration-200 ease-in">
        ...
        <div className="flex flex-col gap-2 justify-between">
          ...
          <button className="w-full bg-[#6D4AFF] text-white font-medium h-14 rounded-lg active:scale-95 transition duration-300 ease-in">Apply Filter</button>
          <button className="w-full border-2 border-[#6D4AFF] active:bg-red-500 text-[#6D4AFF] active:text-white active:border-red-500 active:border-2 font-medium active:scale-95 transition duration-300 ease-in  h-14 rounded-lg">Clear All</button>
